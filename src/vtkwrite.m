...
%  writes a structured 3D vector data into VTK file, with name specified by
%  the string filename. (u,v,w) are the vector components at the points
%  (x,y,z). x,y,z should be 3-D matrices like those generated by meshgrid,
%  where point(ijk) is specified by x(i,j,k), y(i,j,k) and z(i,j,k). The
...
%
%  vtkwrite(filename,'structured_grid',x,y,z,'scalars',title,r) writes
%  a 3D scalar data into VTK file whose name is specified by the string
...
%  vector andscalar values. x,y,z,u,v,w,r must all be the same size and
%   contain the corresponding positon, vector and scalar values.
%
%  vtkwrite(filename, 'structured_points',title, m) saves matrix m
%  (could be 1D, 2D or 3D array) into vtk as structured points.
%
%  vtkwrite(filename, 'structured_points',title, m, 'spacing', sx, sy,
%  sz) allows user to specify spacing. (default: 1, 1, 1). This is the
%  aspect ratio of a single voxel.
%
%  vtkwrite(filename, 'structured_points',title, m, 'origin', ox, oy,
%  oz) allows user to speicify origin of dataset. (default: 0, 0, 0).
%
...
%  contain the corresponding positon, vector and scalar values.
%  
%  vtkwrite(filename, 'polydata','lines', x, y, z) exports a 3D line
%  where x,y,z are coordinates of the points that make the line. x, y, z
%  are vectors containing the coordinates of points of the line, where
%  point(n) is specified by x(n), y(n) and z(n).
%
%  vtkwrite(filename,'polydata','lines',x,y,z,'Precision',n) allows
%  you to specify precision of the exported number up to n digits after
%  decimal point. Default precision is 3 digits.
%
%  vtkwrite(filename,'polydata','triangle',x,y,z,tri) exports a list
%  of triangles where x,y,z are the coordinates of the points and tri is an
%  m*3 matrix whose rows denote the points of the individual triangles.
%
%  vtkwrite(filename,'polydata','tetrahedron',x,y,z,tetra) exports a
%  list of tetrahedrons where x,y,z are the coordinates of the points and
%  tetra is an m*4 matrix whose rows denote the points of individual
%  tetrahedrons.
%
%  vtkwrite(filename,'polydata',...,'facescalar',nb,Name1,Val1,Name2,Val2...)
%  adds values (properties to each of the elements of 'polydata'. 'nb'
%  cointains the number of individual properties that will be added. It is
%  followed by the 'Name' to be shown in PARAVIEW properties and the 'Val'
%  of each property as a vector containing a value for each element of the
%  'polydata'. A different Name and Val is needed for each increment in
%  'nb'
%  
%  vtkwrite('execute','polydata','lines',x,y,z) will save data with
%  default
...
%         fprintf(fid, 'BINARY\n');
        ...
        midx = find(strcmpi(varargin,'MATRIX'));
        if vidx~=0
...
%                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
        ...
        if midx~=0
            for ii = 1:length(midx)
                title = varargin{midx(ii)+1};
                vals=varargin{midx(ii) + 2};
                f=find(ismember(title,'|'));
                for iii=1:size(vals,2)
                    if iii==1
                        t=title(1:f(iii)-1);
                    else
                        t=title((f(iii-1)+1):(f(iii)-1));
                    end
                    fprintf(fid, ['\nSCALARS ', t,' float\n']);
                    fprintf(fid, 'LOOKUP_TABLE default\n');
                    if ~binaryflag
                        spec = ['%0.', precision, 'f '];
                        fprintf(fid, spec, vals(:,iii));
                    else
                        fwrite(fid, vals(:,iii), 'float', 'b');
                    end
                end
            end
        end
    ...
    case 'POLYDATA'
        ...
        switch upper(varargin{1})
            case 'LINES'
                ...
                fprintf(fid,'2 %d %d\n',[conn1';conn2']);
            case 'LINESB'
                nbi =  size(varargin{5},1);
                fprintf(fid,'\nLINES %d %d\n',nbi,3*nbi);
                fprintf(fid,'2 %d %d\n',(varargin{5}-1)'); 
            case 'NOLINES'
                fprintf(fid,'\nLINES 0 0\n');
            case 'TRIANGLE'
                ntri =  size(varargin{5},1);
                fprintf(fid,'\nPOLYGONS %d %d\n',ntri,4*ntri);
                fprintf(fid,'3 %d %d %d\n',(varargin{5}-1)');
            case 'TETRAHEDRON'
                ntetra =  size(varargin{5},1);
                fprintf(fid,'\nPOLYGONS %d %d\n',ntetra,5*ntetra);
                fprintf(fid,'4 %d %d %d %d\n',(varargin{5}-1)');
            case 'PENTAHEDRON'
                npenta = size(varargin{5},1);
                fprintf(fid,'\nPOLYGONS %d %d\n',npenta,6*npenta);
                fprintf(fid,'5 %d %d %d %d %d\n',(varargin{5}-1)');
           case 'HEXAHEDRON'
                nhexa = size(varargin{5},1);
                fprintf(fid,'\nPOLYGONS %d %d\n',nhexa,7*nhexa);
                fprintf(fid,'6 %d %d %d %d %d %d\n',(varargin{5}-1)');
            case 'NOELEMENTS'
                fprintf(fid,'\nPOLYGONS 0 0\n');
        end
        if any(strcmpi(varargin, 'FACESCALAR'))
            ps=find(strcmpi(varargin, 'FACESCALAR'));
            nbscVal=varargin{ps+1};
            if length(varargin)>=(ps+2*nbscVal+1)
                fprintf(fid,'\nCELL_DATA %d\n',numel(varargin{ps+3}));
                for k=1:nbscVal
                    fprintf(fid,"SCALARS "+ string(varargin{ps+2+(k-1)*2})+...
                        " float 1\n");
                    fprintf(fid,'LOOKUP_TABLE default\n');
                    fprintf(fid,'%d %d %d %d %d %d %d\n',varargin{ps+3+(k-1)*2});
                end
            else
                error('FACESCALAR Imput dimesions do not match')
            end
        end
end
fclose(fid);
